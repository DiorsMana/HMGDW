...
D = cellfun(@(x)( 1./sqrt(sum(x, 2)) ), W, 'UniformOutput', false);  %对每个元素归一化操作
D = cellfun(@(x)( x*x' ), D, 'UniformOutput', false);   %对D进行向量积计算，得到正定矩阵
[X, Y, V] = cellfun(@(x)(find(x)), W, 'UniformOutput', false);   %找到W中非零矩阵的位置。


S = cellfun(@(x,y)(x.*y), W, D, 'UniformOutput', false); %W与D对应元素进行相乘。
